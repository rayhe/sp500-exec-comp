...
/* Pre-compute radar chart percentile data — 8 dimensions with S&P 500 and sector percentiles.
   Each company gets c._radarProfile = { comp, stockPct, concPct, payRatio, workerPay, yoyChange, tenure, governance }
...
function computeRadarPercentiles(companies) {
    ...
    var govPctile = rankPercentiles(companies, function(c) { return c._govScore; }, true);
    ...
    companies.forEach(function(c) {
        c._radarProfile = {
            ...
            governance: govPctile[c.ticker] || null
        };
    });
    ...
    Object.keys(sectors).forEach(function(sec) {
        ...
        var dims = ['comp', 'stockPct', 'concPct', 'payRatio', 'workerPay', 'yoyChange', 'tenure', 'governance'];
        var result = {};
    ...
    tbody.addEventListener('click', function(e) {
        ...
        if (company._radarProfile) {
            ...
            var radarDims = [
                ...
                { key: 'governance', label: 'Governance', tip: 'Compensation governance score percentile (SoP, concentration, ratio, team)' }
            ];
            ...
            if (radarValidDims.length >= 4) {
                ...
                html += '<span class="radar-sub">Percentile position across 8 dimensions vs S&P 500 and ' + (company.sector || 'sector') + ' medians</span>';
                html += '</div>';
...
(async function init() {
    ...
    function updatePvpOnlyToggle() {
        ...
        // As of the 2026-09-24 18:00 PT batch all 485 PvP-covered companies
        // are in the main table (IBKR/MRVL/SNDK enriched), so the filter
        // count equals the PvP universe count; no special-casing remains.
        var n;
    ...
    function renderComparisonRadar(selected, parentEl) {
        ...
        var radarDims = [
            ...
            { key: 'governance', label: 'Governance', tip: 'Compensation governance score percentile — higher = stronger governance' }
        ];
        ...
        var radarSortMap = {
            ...
            governance: { key: '_govScore', dir: 'desc' }
        };
    ...
    var METHODOLOGY_CONTENT = {
        ...
        dataq: {
            ...
            html: '<h4>Primary Source: SEC DEF 14A</h4>' +
                ...
                '<li><strong>rounding</strong> — Components don\'t foot the printed total by a small gap; values stored verbatim from the filing. 324 <em>verified</em> rows carry $1&ndash;$2 deltas — that is the filer\'s own rounded-dollar arithmetic, not a parse error, so they stay verified by taxonomy decision (2026-09-10).</li>' +
                ...
                '<p>6,975 of 7,005 NEO rows verified (99.6%): 0 rounding, 0 recomputed, 30 component_mismatch — the remaining rows are honestly labeled, not silently dropped.</p></div>' +
                ...
                '<p id="dataq-payratio-counts">As of the 2026-09-24 screen: 397 of 510 screened companies\' disclosed ratios match <code>total_compensation / median_worker_pay</code> within 3% tolerance; 14 cluster near 2x, 10 near 0.5x, 89 differ otherwise.</p>' +
