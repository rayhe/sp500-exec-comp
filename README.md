...
| Label | Meaning | Rows |
|---|---|---|
| `verified` | Components and total match the filing SCT verbatim. Includes `def14a_verified_YYYYMMDD` re-verification passes | 6,737 (99.4%) |
| `rounding` | Components don't foot the printed total by a small gap; stored verbatim | 24 |
...
Taxonomy decision (2026-09-10): 258 `verified` rows carry $1–$2 deltas between summed components and the printed total (e.g., NVDA/UBER/SOLV FY2025–2026). This is the filer's own rounded-dollar arithmetic, not a parse error, so they stay `verified` — relabeling would cut headline coverage from 99.4% to 95.5% for zero information gain.
